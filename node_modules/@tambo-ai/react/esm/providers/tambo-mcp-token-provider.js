...
import { PLACEHOLDER_THREAD, TamboThreadContext, } from "./tambo-thread-provider";
...
export const TamboMcpTokenProvider = ({ children, }) => {
    ...
    const setThreadMap = useMemo(() => threadContext?.setThreadMap ?? (() => { }), [threadContext?.setThreadMap]);
    ...
    useEffect(() => {
        ...
        const shouldFetchThreadToken = hasThreadChanged &&
            currentThreadId &&
            !isPlaceholderThread &&
            currentThread &&
    ...
    const value = useMemo(() => ({ mcpAccessToken: selectedToken, tamboBaseUrl }), [selectedToken, tamboBaseUrl]);
 ...
 */
export const useTamboMcpToken = (contextKey) => {
    ...
    const [threadlessToken, setThreadlessToken] = useState(null);
    const hasAttemptedFetch = useRef(false);
    // Determine if we should fetch a threadless token
    const isPlaceholderThread = currentThreadId === PLACEHOLDER_THREAD.id;
    const shouldFetchThreadless = contextKey &&
