...
/**
 * Button to attach context files (text, etc.) using useTamboContextAttachment.
 */
export const MessageInputContextAttachmentButton = React.forwardRef<
  HTMLButtonElement,
  MessageInputContextAttachmentButtonProps
>(
  (
    {
      className,
      accept = ".txt,.md,.json,.js,.ts,.tsx,.css,.html,.csv,.xml,.yml,.yaml",
      multiple = true,
      ...props
    },
    ref,
    ...
    return (
      <Tooltip content="Attach Text Context" side="top">
        <button
          ref={ref}
          type="button"
          onClick={handleClick}
...
MessageInputContextAttachmentButton.displayName =
  "MessageInput.ContextAttachmentButton";
...
/**
 * Component that displays active context attachments (text/code files).
 */
export const MessageInputActiveAttachments = () => {
  ...
  return (
    <div className="flex flex-wrap gap-2 pb-2 px-1">
      {attachments.map((att) => (
        <div
          key={att.id}
          className="flex items-center gap-1.5 rounded-md border border-border bg-muted/50 px-2 py-1 text-xs font-medium text-foreground transition-colors hover:bg-muted"
        >
          <FileText className="h-3 w-3 text-muted-foreground" />
          <span className="max-w-[150px] truncate">{att.displayName}</span>
          <button
            onClick={() => removeContextAttachment(att.id)}
